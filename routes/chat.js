...
router.put("/group/add/chat/:id", async (req, res) => {
    ...
    try {
        ...
        isadmin = Admins.map(id => id.toString()).includes(req.user.id.toString());

        if (isadmin) {

            await chat.updateOne({ "_id": new ObjectId(req.params.id) }, { $addToSet: { users: new ObjectId(req.body.userid) } })
    ...
    try{
        ...
        if (isadmin) {
            ...
            await chat.updateOne(
                { _id: new ObjectId(req.params.id) }, {$pull: {users: new ObjectId(req.body.userid)}});
...
router.post("/group/:id/img",upload.single("img"),async(req,res)=>{
    ...
    const token = req.headers.token
       ...
       try{
        ...
        await chat.updateOne({ "_id": new ObjectId(req.params.id) }, {
              $set: {
                "img": {
...
router.put("/group/addadmin/chat/:id",async(req,res)=>{
    ...
    try{
        ...
        if(!isadmin){
            ...
            await chat.updateOne({ "_id": new ObjectId(req.params.id) }, { $addToSet: { admin: new ObjectId(req.body.userid) } })
...
router.put("/group/pulladmin/chat/:id",async(req,res)=>{
    ...
    try{
        ...
        if(!isadmin){
            ...
            await chat.updateOne(
                { _id: new ObjectId(req.params.id) }, {$pull: {admin: new ObjectId(req.body.userid)}});
